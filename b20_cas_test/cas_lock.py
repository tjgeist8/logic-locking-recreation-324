...
class CASLockBuilder:
    ...
    Correct key construction:  K2 = NOT(K1)
        gcas(X)     = cascade(X XOR K1, gate_seq)
        gcas_bar(X) = NOT( cascade( NOT(X XOR K2), gate_seq ) )
                    = NOT( cascade( X XOR K1, gate_seq ) )  [since NOT(K2)=K1]
                    = NOT( gcas(X) )
        Y = gcas AND NOT(gcas) = 0  for ALL inputs  OK

    gcas_bar is built by pre-inverting each keyed input with an INV gate
    before the cascade, then inverting the cascade output. This is equivalent
    to running the same cascade as gcas driven by L1 = X XOR K1.

    K1 and K2=NOT(K1) are separate port groups, so the SAT solver cannot
    trivially identify the correct key relationship from port structure alone.
    """

    def __init__(self, selected_inputs: List[str],
                 gate_seq: List[str], k1: List[int]):
        self.inputs    = selected_inputs
        self.N         = len(selected_inputs)
        self.gate_seq  = gate_seq              # length N-1
        self.k1        = k1                    # length N, key for gcas half
        self.k2        = [1 - b for b in k1]  # K2 = NOT(K1), key for gcas_bar half
        self._uid      = 0
    ...
    def _xor_key(self, in_net: str, key_index: int) -> str:
        out = self._w("xk")
        self._emit("XOR2_X1", self._iname("xor"), A=in_net, B=f"keyinput{key_index}", Z=out)
        return out
    ...
    def build(self) -> Tuple[List[str], List[str], str]:
        ...
        keyed_g = [self._xor_key(self.inputs[i], i) for i in range(self.N)]
        g_out   = self._cascade(keyed_g, "g")

        # gcas_bar half  (key ports: keyinput_N .. keyinput_{2N-1})
        # Pre-invert each keyed input before the cascade, then invert the
        # cascade output. This implements gcas_bar(X) = NOT(cascade(NOT(X XOR K2)))
        # = NOT(cascade(X XOR K1)) = NOT(gcas(X)), ensuring Y=0 for correct key.
        keyed_gb = [self._xor_key(self.inputs[i], self.N + i) for i in range(self.N)]
        pre_inv_gb = []
        for net in keyed_gb:
            inv_out = self._w("gb_preinv")
            self._emit("INV_X1", self._iname("gb_preinv"), A=net, ZN=inv_out)
            pre_inv_gb.append(inv_out)
        gb_cascade_out = self._cascade(pre_inv_gb, "gb")

        # Invert cascade output to complete gcas_bar = NOT(cascade(NOT(L2)))
    ...
    def correct_key_bits(self) -> List[int]:
        return list(self.k1) + list(self.k2)  # K1 then K2=NOT(K1)
...
def verify_correct_key(gate_seq: List[str], N: int, k1: List[int]) -> bool:
    """Y must be 0 for all 2^N patterns when K2=NOT(K1)."""
    k2 = [1 - b for b in k1]
    ...
    for pat in range(1 << N):
        bits = [(pat >> i) & 1 for i in range(N)]
        L1    = [bits[i] ^ k1[i] for i in range(N)]
        L2    = [bits[i] ^ k2[i] for i in range(N)]
        notL2 = [1 - b for b in L2]
        g  = cascade(L1)
        gb = 1 - cascade(notL2)   # NOT(cascade(NOT(L2)))
        if g & gb:
            return False
    return True
...
def emit_locked_verilog(module: VerilogModule,
    ...
    lines += [
        ...
        f"//  Correct key (bits)   : K1={k1}  K2={[1-b for b in k1]}",
...
def apply_cas_lock(module: VerilogModule,
    ...
    if len(k1) != actual_N:
        raise ValueError(f"Key K1 must have {actual_N} bits, got {len(k1)}")
    ...
    # Sanity check
    if not verify_correct_key(gate_seq, actual_N, k1):
        raise RuntimeError("INTERNAL ERROR: correct key check failed!")

    # Build CAS-Lock block
    builder        = CASLockBuilder(selected, gate_seq, k1)
    ...
    return emit_locked_verilog(
        ...
        actual_p, actual_N, k1, correct_key_2n,
...
def print_key_report(module_name, N, k1, key_ports, correct_key_2n,
    ...
    k2 = [1 - b for b in k1]
    print(f"    K1          : {' '.join(str(b) for b in k1)}")
    print(f"    K2=NOT(K1)  : {' '.join(str(b) for b in k2)}")
    print(f"    Full 2N-key : {' '.join(str(b) for b in k1)} | {' '.join(str(b) for b in k2)}")
    ...
    for i, (port, bit) in enumerate(zip(key_ports, correct_key_2n)):
        ...
        print(f"  {port:<22s}  {bit}             {role}  ->  keyinput{i}")
    print()
...
def main():
    ...
    # Parse K1 bits from the header comment "K1=[0, 1, ...]"
    m_k1 = re.search(r'K1=\[([\d,\s]+)\]', locked)
    final_k1   = [int(b.strip()) for b in m_k1.group(1).split(',')] if m_k1 else []
    final_k2   = [1 - b for b in final_k1]
    final_key  = final_k1 + final_k2
    ...
    final_N    = len(final_k1) if final_k1 else N

    print_key_report(module.name, final_N, final_k1, final_ports,
