...
def validate_with_abc(bench_file):
    ...
    try:
        ...
        if "Error" in output or "failed" in output.lower() or "Cannot open" in output:
            print("âŒ Validation FAILED. ABC rejected the BENCH file:")
            # Print only the relevant error lines from ABC
            for line in output.split('\n'):
                if "Error" in line or "failed" in line.lower():
                    print(f"   {line.strip()}")
        else:
            print("âœ… Validation SUCCESS. ABC successfully parsed the file topology.")
        ...
        print("âš ï¸ Warning: Could not find 'yosys-abc' or 'abc' in your system PATH. Skipping validation.")
    except Exception as e:
        print(f"âš ï¸ An error occurred during validation: {e}")
